...
ScavTrap::ScavTrap(std::string n) : ClapTrap(n)
{
    ...
    energy_point = 50;
    attack_damage = 20;
    std::cout << "Name constractor ScavTrap is called\n";
}
...
void ScavTrap::attack(const std::string& target)
{
    ...
    {
        energy_point--;
        std::cout << "PPRRRRR.. ClapTrap " << name << " attacks " << target << ", causing " << attack_damage << " points of damage!\n";
    }
    if (energy_point == 0)
        std::cout << "OOOEPS ClapTrap " << name << " run out of energy points \n";
    if (hit_points == 0)
        std::cout << "UUUH ClapTrap " << name << " cannot attack is already dead!\n";
}

void ScavTrap::guardGate()
{
    std::cout << "ClapTrap" << name << "ScavTrap is now in Gate keeper mode.\n";
}